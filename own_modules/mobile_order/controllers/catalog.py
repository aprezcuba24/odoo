...
class MobileCatalogController(http.Controller):
    ...
    @mobile_auth(require_pos_config=True)
    def mobile_products(self, mobile_pos_config=None, mobile_product_domain=None, **kwargs):
        ...
        extend_domain_from_int_params(domain, request.params, 'category_id', 'product_tmpl_id.categ_id', 'child_of')
        extend_domain_from_int_params(domain, request.params, 'pos_category_id', 'product_tmpl_id.pos_categ_ids', 'child_of')
        ...
        return mobile_json_response(serialize_pagination(data, limit, offset, total, mobile_pos_config.id))

    @http.route('/api/mobile/products/<int:product_id>', type='http', auth='public', methods=['GET', 'OPTIONS'], csrf=False)
    @mobile_auth(require_pos_config=True)
    def mobile_product_detail(self, product_id, mobile_pos_config=None, mobile_product_domain=None, **kwargs):
