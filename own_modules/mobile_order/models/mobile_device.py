...
def normalize_phone_for_mobile(env, phone_raw):
...
class MobileDevice(models.Model):
    ...
    def register_or_get(self, phone_raw, device_key, name, device_info=None):
        ...
        normalized = normalize_phone_for_mobile(self.env, phone_raw)
        if not normalized:
    ...
    def cron_deactivate_inactive_devices(self):
        ...
        try:
            days = int(ICP.get_param('mobile_order.device_inactivity_days', '90'))
        except ValueError:
            days = 90
        if days <= 0:
            return
