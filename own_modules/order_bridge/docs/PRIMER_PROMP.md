...
overview: Historical design brief. The implemented addon is **`order_bridge`** (neutral REST API for external clients). See [ARCHITECTURE.md](ARCHITECTURE.md) for the current model names, routes under `/api/order_bridge/`, and upgrade notes from the former `mobile_order` addon.
todos:
  - id: create-docs
