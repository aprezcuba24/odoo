...
- **Multi-company** (separate Railway project): one PostgreSQL DB, many `res.company`; self-service via `company_onboarding`; API uses `company_slug` / `X-Company-Slug`. See `docs/RAILWAY_MULTI_COMPANY_CHECKLIST.md`. Do not enable on single-tenant production.
